"""混合回放数据集构建：任意任务按帧数上限混合（像素级回放的载体）。

用法：
    export STABLEWM_HOME=/root/autodl-tmp/.stable_worldmodel
    python cl_protocol/merge_replay.py \
      --sources pusht_expert_train.lance:20000 pusht_t2.lance \
      --out pusht_t2_replay

输出：<STABLEWM_HOME>/datasets/<out>.lance

设计（与 merge_joint.py 同模式）：
  - --sources 每项 = "数据集名:帧数上限" 或 "数据集名"（全取），按整 episode 切
  - 列取各数据集 schema 交集（以第一个源为准排序）
  - episode_idx 重编号，保持 episode-contiguous（LanceDataset 硬性要求）
  - 像素列是 JPEG blob，全程 pyarrow 流式复制，无解码/重编码损失

回放比例说明：T2 微调集约 10 万帧；掺 2 万帧 T1 = 约 17% 回放占比，
训练步数/epoch 从 639 升到约 767（+20% 时长）。
"""
import argparse
import sys
...
def main():
    ...
    ap.add_argument('--sources', required=True, nargs='+',
                    help='数据集名:帧数上限 或 数据集名（全取），可多个')
    ap.add_argument('--out', required=True, help='输出数据集名（不含 .lance）')
    args = ap.parse_args()
    sources = [parse_source(s) for s in args.sources]

    datasets_dir = swm.data.utils.get_cache_dir(sub_folder='datasets')
    out_dir = datasets_dir / f'{args.out}.lance'
    if out_dir.exists():
    ...
    common = [f.name for f in schemas[0]
              if all(f.name in s.names for s in schemas[1:])]
    for col in ('episode_idx', 'step_idx', 'pixels', 'action'):
        assert col in common, f'关键列 {col} 不在各数据集交集中: {common}'
    ...
    # --- 验收 ---
    out = open_table(datasets_dir, f'{args.out}.lance')
